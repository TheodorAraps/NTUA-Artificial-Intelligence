	...
	def __init__(self, N, S, F):
		...
		while not stack.empty():
			...
			if len(neighbour) > 0:
				...
				if cr_path_flag: # Όσο δεν έχουμε βρεί το critical path. πρόσθεσε το κελί σαν πιθανό μέρος του critical path
					critical_path.append((X, Y))

				if (X, Y) == self.F: # Άμα φτάσουμε στον F βρήκαμε το critical path (ανά δύο κελιά, θα λείπουν τα ενδιάμεσα)
					cr_path_flag = False 
			...
			if cr_path_flag:
				critical_path.remove((x, y)) # Διατηρούμε μόνο τα κελιά που αποτελούν μέρος της διαδρομής από τον S στον F (αφαιρούμε τα περιττά)
			...
			"""
		# Προσθέτουμε στο critical path όλα τα ενδιάμεσα που λείπουν
		for i in range(len(critical_path) - 1):
			x, y = critical_path[i]
			x_next, y_next = critical_path[i + 1]
			x_diff = x - x_next
			y_diff = y - y_next
			if x_diff < 0:
				x = x + 1
		...
		cr_path_set = set(critical_path) # Το μετατρέπουμε σε set για να κάνουμε γρηγορότερες αναζητήσεις
		flag = True # Boolean μεταβλητή προκειμένου να επιβεβαιώσουμε ότι δημιουργείται όντως τουλάχιστον ένα καινούριο μονοπάτι από τον S στον F
		for i in range(self.N//2): # Ο αριθμός που βάλουμε μέσα στο range καθορίζει τον αριθμό των τοίχων που "γκρεμίζονται"
			while (1):
				...
				if flag == True: # Θα επαληθευτεί μόνο στην πρώτη επανάληψη του for loop
					if (x - 1, y) in cr_path_set or (x + 1, y) in cr_path_set or (x, y - 1) in cr_path_set or (x, y + 1) in cr_path_set: # Εξετάζουμε αν o τοίχος είναι ανάμεσα από δύο κελιά που ανήκουν στο critical path
						flag = False
					else:
						continue
				break
